...
The email sending mechanism is supported by the https://docs.oracle.com/javaee/7/api/javax/mail/package-summary.html[`JavaMail`] library. This feature is composed of three commands: `check`, `setemail` and `remind`.

* `check` command: display the email address that the user has set to send emails from. The app will censor the email address.
[NOTE]
...
*Step 4.* The user forgets what email he has set before, so he executes `check` again. It has the same procedure as *Step 2*. However, the app detects that user's email is not `"default"` this time, so it creates an `Email(userEmail)` object and executes `Email#getCensored()`. Then, the function returns a `CommandResult` with a success message and the censored user email.
...
** Pros: The user has a choice of what email address to use. +
The user needs to type in the correct password if they want to send acreminder email, which ensures the security and privacy of the email.

** Cons: The user might need to enable the `less secure apps` setting, which could be a security concern.

* *Alternative 2:* Use a common email provided by the app.

** Pros: The user does not need to set their own email when sending reminder emails, reducing hassle when setting up. +
The user does not need to type in a password when sending a reminder email.

** Cons: The default email is less secure, as its password can be found within the program. +
The password of this email can be changed by malicious users. +
The email might get overloaded. +
Customer might receive spam emails, as a email password is not required.
...
Expected: Receive a success message "Email sent!" on the screen. A reminder email should have been sent from the sender's email (check the Sent folder). A reminder email should be in the inbox of the customer's email account that you set.
