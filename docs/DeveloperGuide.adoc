...
*Step 2.* The user executes `checkemail`. The command executes `Model#getMyEmail()`, which calls `UserPrefs#getDefaultEmail()` and returns user's email stored in `UserPref`. The app checks if user's email equals to `"default"`. In this case, they are equal, so it throws a `CommandException`.

The following sequence diagram shows how `checkemail` works:
...
.. Do several equal checks:

... Check if `OLDEMAIL` equals to `"default"` or the email the user set last time.

... If they are equal, then check if `OLDEMAIL` equals to `NEWEMAIL`.

... If they are not equal, then call `SetEmailCommand#isValidGmail(NEWEMAIL)` to check if `NEWEMAIL` is a valid gmail.

.. If it returns true, call `ModelManager#setMyEmail(NEWEMAIL)` and this method calls `UserPrefs#setDefaultEmail(NEWEMAIL)` to set `NEWEMAIL` as user's email and store it in `preferences.json`.
[NOTE]
In step a, if the next step cannot be executed, the app throws a `CommandException` with corresponding failure message.

*Step 4.* The user forgets what email he has set before, so he executes `checkemail` again. It has the same procedure as *Step 2*. However, the app detects that user's email is not `"default"` this time, so it creates an `Email(userEmail)` object and executes `Email#getCensored()`. Then, the function returns a `CommandResult` with a success message and the censored user email.
...
** Cons:
*** The user might need to enable the `less secure apps` setting, which could be a security concern.
...
** Cons:
*** The default email is less secure, as its password can be found within the program.
...
* Execute `checkemail` command. Then use the `setemail` command to set the sender's email.
