...
* `LoanListPanel#setGraphic()` -- call the `LoanCard` class which will create a LoanCard containing information of the new loan.

* `LoanCard#LoanCard()` -- the constructor will assign values to each of the labels shown in the LoanCard. However, the value of Nric, phone number and email will be censored and hided, which means getCensor() will be called at this time, before they are assigned to their corresponding labels.
...
As the cendored data are only hidden from the window, not changed in the database. The `BrowserPanel` on the bottom right of the window will still show all the information without censoring after users click the `LoanCard` for more detailed information about the loan.
