...
[NOTE]
As a more OOP model, we can store a `Tag` list in `Address Book`, which `Loan` can reference. This would allow `Address Book` to only require one `Tag` object per unique `Tag`, instead of each `Loan` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
...
Step 3. The user executes `add n/David ...` to add a new loan. The `add` command also calls `Model#commitAddressBook()`, causing another modified address book state to be saved into the `addressBookStateList`.
...
User must follow the input format when adding a new loan, which is `add p/PERSON_NAME ic/NRIC hp/PERSON_PHONE_NO e/PERSON_EMAIL r/RATE t/TIME o/OBJECT_IDENTITY`.

However, some of the information is very important and personal, e.g. `Nric`, so it is irresponsible and dangerous to show these data directly on the `LoanListPanel` of the user interface.

As a result, we add a new feature to hide the private information from the Window. +
Example: the input `Nric` and stored in the database is `Nric: G1234567U`, but our `LoanCard` will only show `Nric: Gxxxxx67U`.

image::LoanListPanelView.png[]

==== Current implementation

The Hide private information is facilitated by Interface `Censor`. It is implemented by classes `Nric`, `Phone` and `Email`.
...
The `LoanCard` object will contain information of the loan: `LoanID`, `BikeID`, `Name`, `Nric`, `Phone`, `Email`, `LoanRate` and `LoanTime`.

**Step2.** Assign the value of the each components to their corresponding labels. +
For example: `name.setText(loan.getName().value);` will directly assign the name String of this loan to the `name` label. +

However, the values of `Nric`, `Phone` and `Email` need censoring before assigning their values. +
For example: `phone.setText(loan.getPhone().getCensored().value);` will censor the value of the phone String of this loan and then assign the value to the `phone` label.

**Step3.** This step actually happens at the same time with **Step2**. +
`Nric`, `Phone` and `Email` will call their corresponding `getCensored()` in their own class and return the censored String.
...
==== Where to see the hided information

As the censored data are only hidden from the window, not changed in the database. The `BrowserPanel` on the bottom right of the window will still show all the information without censoring after users click the `LoanCard` for more detailed information about the loan.
...
. Add a shorthand equivalent alias for each of the individual commands. For example, besides typing `clear`, the user can also type `c` to remove all loans in the list.
+
****
* Hints
...
* Solution
** Implement a `removeTag(Tag)` method in link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`]. Loop through each loan, and remove the `tag` from each loan.
...
. Use different colors for different tags inside loan cards. For example, `friends` tags can be all in brown, and `colleagues` tags can be all in yellow.
+
**Before**
+
...
* Solution
** You can modify the existing test methods for `LoanCard` 's to include testing the tag's color as well.
...
* Solution
** Modify the constructor of link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] to take in the number of loans when the application just started.
** Use link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter#handleAddressBookChangedEvent(AddressBookChangedEvent)`] to update the number of loans whenever there are new changes to the addressbook.
** For tests, modify link:{repoURL}/src/test/java/guitests/guihandles/StatusBarFooterHandle.java[`StatusBarFooterHandle`] by adding a state-saving functionality for the total number of people status, just like what we did for save location and sync status.
** For system tests, modify link:{repoURL}/src/test/java/systemtests/AddressBookSystemTest.java[`AddressBookSystemTest`] to also verify the new total number of loans status bar.
...
==== Description
Edits the remark for a loan specified in the `INDEX`. +
Format: `remark INDEX r/[REMARK]`

Examples:

* `remark 1 r/Likes to drink coffee.` +
Edits the remark for the first loan to `Likes to drink coffee.`
* `remark 1 r/` +
Removes the remark for the first loan.
...
===== [Step 3] Ui: Add a placeholder for remark in `LoanCard`
Let's add a placeholder on all our link:{repoURL}/src/main/java/seedu/address/ui/LoanCard.java[`LoanCard`] s to display a remark for each loan later.

**Main:**

. Add a `Label` with any random text inside link:{repoURL}/src/main/resources/view/LoanListCard.fxml[`LoanListCard.fxml`].
. Add FXML annotation in link:{repoURL}/src/main/java/seedu/address/ui/LoanCard.java[`LoanCard`] to tie the variable to the actual label.

**Tests:**

. Modify link:{repoURL}/src/test/java/guitests/guihandles/LoanCardHandle.java[`LoanCardHandle`] so that future tests can read the contents of the remark label.

===== [Step 4] Model: Add `Remark` class
We have to properly encapsulate the remark in our link:{repoURL}/src/main/java/seedu/address/model/loan/Loan.java[`Loan`] class. Instead of just using a `String`, let's follow the conventional class structure that the codebase already uses by adding a `Remark` class.

**Main:**

. Add `Remark` to model component (you can copy from link:{repoURL}/src/main/java/seedu/address/model/loan/Address.java[`Address`], remove the regex and change the names accordingly).
...
===== [Step 5] Model: Modify `Loan` to support a `Remark` field
Now we have the `Remark` class, we need to actually use it inside link:{repoURL}/src/main/java/seedu/address/model/loan/Loan.java[`Loan`].

**Main:**

. Add `getRemark()` in link:{repoURL}/src/main/java/seedu/address/model/loan/Loan.java[`Loan`].
. You may assume that the user will not be able to use the `add` and `edit` commands to modify the remarks field (i.e. the loan will be created without a remark).
. Modify link:{repoURL}/src/main/java/seedu/address/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete your `addressBook.xml` so that the application will load the sample data when you launch it.)

===== [Step 6] Storage: Add `Remark` field to `XmlAdaptedLoan` class
We now have `Remark` s for `Loan` s, but they will be gone when we exit the application. Let's modify link:{repoURL}/src/main/java/seedu/address/storage/XmlAdaptedLoan.java[`XmlAdaptedLoan`] to include a `Remark` field so that it will be saved.
...
. Fix `invalidAndValidLoanAddressBook.xml`, `typicalLoansAddressBook.xml`, `validAddressBook.xml` etc., such that the XML tests will not fail due to a missing `<remark>` element.

===== [Step 6b] Test: Add withRemark() for `LoanBuilder`
Since `Loan` can now have a `Remark`, we should add a helper method to link:{repoURL}/src/test/java/seedu/address/testutil/LoanBuilder.java[`LoanBuilder`], so that users are able to create remarks when building a link:{repoURL}/src/main/java/seedu/address/model/loan/Loan.java[`Loan`].

**Tests:**

. Add a new method `withRemark()` for link:{repoURL}/src/test/java/seedu/address/testutil/LoanBuilder.java[`LoanBuilder`]. This method will create a new `Remark` for the loan that it is currently building.
. Try and use the method on any sample `Loan` in link:{repoURL}/src/test/java/seedu/address/testutil/TypicalLoans.java[`TypicalLoans`].

===== [Step 7] Ui: Connect `Remark` field to `LoanCard`
Our remark label in link:{repoURL}/src/main/java/seedu/address/ui/LoanCard.java[`LoanCard`] is still a placeholder. Let's bring it to life by binding it with the actual `remark` field.

**Main:**

. Modify link:{repoURL}/src/main/java/seedu/address/ui/LoanCard.java[`LoanCard`]'s constructor to bind the `Remark` field to the `Loan` 's remark.

**Tests:**

. Modify link:{repoURL}/src/test/java/seedu/address/ui/testutil/GuiTestAssert.java[`GuiTestAssert#assertCardDisplaysLoan(...)`] so that it will compare the now-functioning remark label.
...
. Replace the logic in `RemarkCommand#execute()` (that currently just throws an `Exception`), with the actual logic to modify the remarks of a loan.
...
3.  User requests to delete a specific loan in the list
...
2.  LoanBook adds the loan and displays a prompt to the user indicating success.
+
Use case ends.
...
3.  User requests to returns a specific loan from the list
4.  LoanBook marks the loan as returned, and based on the prevailing rates and rental times in the loan book, prompts the user to charge the customer a certain amount of money.
+
Use case ends.
...
+
[none]
** 3a1. LoanBook shows an error message giving the user an example command, as well as correct format of the command.
+
Use case ends.
