...
The sending email mechanism is supported by `Javax Mail` library. This feature is composed by three commands, which are `check`, `setemail` and `remind`.

* `check` command: check for the personal email the user has set and the app will show the censored email address.
[NOTE]
If user has not set an email, the email address will be a string "default" and the app will throw a `CommandException`.

* `setemail` command: set the personal email address used for sending reminder to the customers.
[NOTE]
Only gmail is accepted by the app.

* `remind` command: automatically generate an email containing some information of the loan and send it to the corresponding customer from user's email. This command will use the `Javax Mail` library.
[NOTE]
The email contains `Name`, `BikeId`, `LoanStartTime`, `LoanRate` of the loan.

Given below is an example usage scenario and how the sending a reminder email mechanism behaves at each step.

*Step 1.* The user launches the application for the first time. User's email address is string "default" by default.
...
*Step 3.* The user executes `setemail OLDEMAIL NEWEMAIL`. This command implements the following operations:

a. Check if `OLDEMAIL` equals to "default" or the email the user set last time.

b. If they are equal, then check if `OLDEMAIL` equals to `NEWEMAIL`.

c. If they are not equal, then call `SetEmailCommand#isValidGmail(NEWEMAIL)` to check if `NEWEMAIL` is a valid gmail.

d. If it returns true, call `ModelManager#setMyEmail(NEWEMAIL)` and this method calls `UserPrefs#setDefaultEmail(NEWEMAIL)` to set `NEWEMAIL` as user's email and store it in `preferences.json`.
[NOTE]
For step a, b and c, if the next step cannot be executed, the app throws a `CommandException` with corresponding failure message.

*Step 4.* The user forgets what email he set before, so he executes `check` again. It has the same procedure as *Step 2*. However, the app detects that user's email is not "default" this time, so it creates an `Email(userEmail)` object and executes `Email#getCensored()`. Then, the function returns a `CommandResult` with a success message and the censored user email.

*Step 5.* The user wants to send an reminder email to a customer by executing `remind x/EMAILPASSWORD n/NAME b/BIKEID`. This command implements the following operations:

a. Get a list of `Loans` from `LoanBook` class and executes `RemindCommand#getLoan(list, NAME, BIKE)` to check if there is a `loan` that contains both this `NAME` and `BIKE`.
...
[NOTE]
If the user cannot remember the customer's `NAME` or the loaned `BIKE`, he can use `find [NAME] [BIKE]` to search for the target loan.
[NOTE]
For Step a and b, if the next step cannot be executed, a `CommandException` with corresponding failure message will be thrown.
[NOTE]
If it throws a `CommandException` with authentication failed message in step c, please check if user's email and its password are correct. If they are correct, please make sure the `less secure apps` setting of user's email is enabled. If it is not enabled, enable it and refresh the page.

==== Design considerations
*Aspect: use user's personal email vs share an immutable common email*

* *Alternative 1 (current choice):* use user's own email

** Pros: The user can have multiple choice when setting their personal email. +
The user needs to type in the correct email password if they want to send reminder email, which ensures the security and privacy of the email.

** Cons: The user might encounters troubles when enabling `less secure apps` setting and this might make user's email less secure.

* *Alternative 2:* every user shares the same email

** Pros: The user does not need to set their own email and do the setting before sending reminder email. +
The format of `remind` command can be changed to `remind n/NAME b/BIKE`, which means the user do not bother to type in email's password, because it is stored in the app.

** Cons: The security of the default email can be threatened, because the password of the email can be found in the program. +
The password of this email can be changed by malicious users. +
The email might be overloaded. +
Customer might receive spam emails, because email password is not required.

==== Future function
We plan to add a feature that can automatically send an e-receipt to the customer after `return` a loan.

The e-receipt will contain `Name`, `BikeID`, `LoanStartTime`, `LoanEndTime` and `TotalCost` of the loan.
...
Prerequisites: Connect to the web. The email must be gmail and its `less secure apps` is enabled.

. Send a reminder email successfully.

`Add` a loan that contains your another email. Execute `check` command. Then use `setemail` command to set the correct email or `find [NAME]` to find the loan you just added if necessay. After that, execute `remind x/correctpassword n/Name b/BikeId`. +
Expected: Receive a success message "Email sent!" on the screen. Login your sent-from email, a reminder email exists in the sent box. Login your sent-to email, you received a reminder email.
...
Expected: Failure message "Your new email address is invalid! It must be valid gmail!" will show. User's email is not changed.
...
Expected: Failure message "You do not need to send reminder, because the loan is RETURNED" will show. The email is not sent.
.. Test case: `remind x/CORRECTPASSWORD n/DELETED_NAME n/DELETED_BIKE` +
Expected: Failure message "You do not need to send reminder, because the loan is DELETED" will show. The email is not sent.
